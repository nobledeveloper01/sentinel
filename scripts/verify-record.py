#!/usr/bin/env python3
"""Verify a Sentinel alert-record export with nothing but Python.

    python3 scripts/verify-record.py alert.txt [expected-public-key-hex]

The file ends with two lines: the phone's Ed25519 public key and the
...
def main(path, expected=None):
    ...
    if verify(bytes.fromhex(pk[1]), bytes.fromhex(sig[1]), body):
        rows = body.count(b"\n") - 1
        print(f"✓ signature holds over {rows} events; key {pk[1][:16]}…")
        return 0
